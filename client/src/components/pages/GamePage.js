...
class GamePage extends Component {
    ...
    componentDidMount() {
        ...
        if (email) {
            ...
            socket.on('waiting-reconnect', msg =>
                this.setState({
                    message: msg,
                    playerJoined: false,
                    rounds: [false, false, false]
        ...
        socket.on('game-start', name => {
            this.setState({
                ...
                message: '',
                opName: name,
                info: 'Who ever enter the word correctly first wins!'
            });
    ...
    submit = () => {
        const { rounds, socket, guest } = this.state;
        ...
        if (email) {
            ...
            socket.emit('round-1-end', guest);
        } else if (rounds[1]) {
            socket.emit('round-2-end', guest);
        } else if (rounds[2]) {
            socket.emit('round-3-end', guest);
        }
    };
    ...
    render() {
        ...
        return (
            <div className="gameContainer">
                <Grid id="gameGrid" centered columns={2} stackable>
                    <Grid.Row>
                        ...
                        <Grid centered>
                            <Grid.Row>
                                <Grid.Column>
                                    ...
                                    <Divider />
                                    {playerJoined && rounds[0] && (
                                        <h1> {word}</h1>
                                    )}
                                    {playerJoined && rounds[1] && (
                                        <h1> {word}</h1>
                                    )}
                                    {playerJoined && rounds[2] && (
                                        <h1> {word}</h1>
                                    )}
                                </Grid.Column>
                            </Grid.Row>
                    ...
                    <Grid.Row>
                        <Grid.Column>
                            {!playerJoined && !playerDisconnected && (
                                <Loader active>{message}</Loader>
                            )}
                            {playerJoined && (
                                <OponentForm textStream={textStream} />
                            )}
                            {playerJoined && <h1>{opName}</h1>}
                            {gameEnd && win && email && (
                                <h2 style={{ color: 'red' }}>-10 Points</h2>
                            )}
                            {gameEnd && lose && email && (
                                <h2 style={{ color: 'green' }}>+30 Points</h2>
                            )}
                        </Grid.Column>
                        <Grid.Column>
                            {message && win && !gameEnd && (
