...
router.post("/update_password", function (req, res) {
    ...
    _jsonwebtoken2.default.verify(token, process.env.JWT_SECRET, function (err) {
        if (err) {
            res.status(401).json({ errors: { global: "Invalid token" } });
        } else {
            _User2.default.findOneAndUpdate({ resetPasswordToken: token }, { resetPasswordToken: "" }, { new: true }).then(function (user) {
                if (user) {
                    user.setPassword(password);
                    user.save().then(function (user) {
                        (0, _mailer.sendPasswordChangedEmail)(user, password);
                        res.json({});
                    });
                } else {
